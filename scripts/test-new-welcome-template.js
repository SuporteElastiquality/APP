// Teste do novo template de email de boas-vindas com logo
const { Resend } = require('resend')

const resend = new Resend('re_BkdZuoJY_DuGJw5VbWroACMMxEL8MnxF9')

async function sendWelcomeEmailWithLogo(email, name) {
  try {
    ...
    console.log('✅ Email de boas-vindas com logo enviado com sucesso para:', email)
    return { success: true }
  } catch (error) {
...
async function testNewTemplate() {
  console.log('📧 Testando novo template de email de boas-vindas com logo...\n')
  ...
  for (const user of testUsers) {
    console.log(`🧪 Enviando email com novo template para: ${user.email}`)
    
    try {
      const result = await sendWelcomeEmailWithLogo(user.email, user.name)
      
      if (result.success) {
        console.log(`   ✅ Email enviado com sucesso!`)
        console.log(`   📧 Verifique a caixa de entrada de ${user.email}`)
        console.log(`   🖼️ O email agora inclui o logo da Elastiquality`)
      } else {
