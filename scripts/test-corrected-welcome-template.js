// Teste do template corrigido de email de boas-vindas
const { Resend } = require('resend')

const resend = new Resend('re_BkdZuoJY_DuGJw5VbWroACMMxEL8MnxF9')

async function sendWelcomeEmailCorrected(email, name) {
  try {
        ...
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
          ...
          <div style="background: #F8FAFC; padding: 30px; border-radius: 10px; margin-bottom: 20px;">
            ...
            <div style="text-align: center; margin: 30px 0;">
              <a href="https://appelastiquality-rpimnnckw-suporte-elastiquality.vercel.app" 
    ...
    console.log('✅ Email de boas-vindas corrigido enviado com sucesso para:', email)
    return { success: true }
  } catch (error) {
...
async function testCorrectedTemplate() {
  console.log('📧 Testando template corrigido de email de boas-vindas...\n')
  console.log('🖼️ Layout: "Bem-vindo à" + Logo da Elastiquality\n')
  ...
  for (const user of testUsers) {
    console.log(`🧪 Enviando email corrigido para: ${user.email}`)
    
    try {
      const result = await sendWelcomeEmailCorrected(user.email, user.name)
      
      if (result.success) {
        console.log(`   ✅ Email enviado com sucesso!`)
        console.log(`   📧 Verifique a caixa de entrada de ${user.email}`)
        console.log(`   🖼️ Layout: "Bem-vindo à" + Logo`)
      } else {
